...
export default function EditInvestorModal({ isOpen, onClose, onSuccess, investor }: EditInvestorModalProps) {
  ...
  useEffect(() => {
    if (investor) {
      setFormData({
        name: investor.name || '',
        capital: investor.capital !== undefined ? investor.capital.toString() : '',
        type: investor.type || 'retail'
      });
    }
  }, [investor]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || 'فشل تعديل بيانات المستثمر');
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-[#141414] border border-[#262626] rounded-xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        <div className="flex justify-between items-center p-5 border-b border-[#262626] bg-[#0f0f0f]">
          <h2 className="text-xl font-bold text-white">تعديل بيانات المستثمر</h2>
          <button 
            onClick={onClose}
